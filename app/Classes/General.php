...
class General
{
    ...
    public function check_htaccess()
    {
        ...
        if (isset($settings['https']) && $settings['https'] == true) {
            $pathExm = implode(DIRECTORY_SEPARATOR, [__DIR__, '..', 'htaccess.https.example']);
        }
    ...
    {
        $path = implode(DIRECTORY_SEPARATOR, [__DIR__, '..', '..', 'robots.txt']);
        $pathExm = implode(DIRECTORY_SEPARATOR, [__DIR__, '..', 'robots.example']);
        if (!file_exists($path)) {
            $raw = file_get_contents($pathExm);
            file_put_contents($path, $raw);
        }
